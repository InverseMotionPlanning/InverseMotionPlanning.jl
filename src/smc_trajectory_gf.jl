...
function Gen.update(
    ...
    # Infer how new chosen trace could have been sampled
    selection = AllSelection()
    ...
    if all(diff isa NoChange for diff in argdiffs) || args == trace.args
    ...
    new_log_z_est = log(new_w_sum) - log(gen_fn.n_particles)
